  ...
  try {
    ...
    if (entry.type === "Holiday" || entry.type === "Shutdown") {
      ...
      if (affected.length > 0) {
        ...
        affectedJobIds = [
          ...new Set(affected.map((e) => e.jobOrderId.toString())),
        ];
      ...
      const targetMachines =
        await MachineMaster.find(machineFilter).select("_id");
    ...
    res
      .status(201)
      .json({ ...entry.toObject(), affectedJobs: affectedJobIds.length });
  } catch (error) {
    res
      .status(500)
      .json({ error: "Failed to create calendar entry: " + error.message });
  }
};

exports.update = async (req, res) => {
  try {
    const entry = await FactoryCalendar.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true },
    );
