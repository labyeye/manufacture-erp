...
exports.getAll = async (req, res) => {
  try {
    ...
    const logs = await BreakdownLog.find(query)
      .populate("machineId")
...
exports.create = async (req, res) => {
  try {
    ...
    if (affectedEntries.length > 0) {
      ...
      const uniqueJobIds = [
        ...new Set(affectedEntries.map((e) => e.jobOrderId.toString())),
      ];
    ...
    await MachineMaster.findByIdAndUpdate(log.machineId, {
    ...
    res
      .status(500)
      .json({ error: "Failed to create breakdown log: " + error.message });
  }
};

exports.update = async (req, res) => {
  try {
    const log = await BreakdownLog.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
