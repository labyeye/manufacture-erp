...
function buildVelocityMap(jobOrders) {
  ...
  (jobOrders || []).forEach((jo) => {
    ...
    if (jo.paperType)
      add(mkey(jo.paperType, jo.paperGsm), estimateJobKg(jo, 1), date);
...
const fmtKg = (n) =>
  n == null || isNaN(n) || n === 0 ? "—" : `${Number(n).toFixed(1)} kg`;
...
function DraftPOModal({
  ...
  const defaultQty =
    row.v30 > 0 ? Math.max(0, targetDays * row.v30 - row.currentKg) : 0;

  const [vendorName, setVendorName] = useState("");
  const [qty, setQty] = useState(
    fmtN(defaultQty, 1) === "—" ? "" : fmtN(defaultQty, 1),
  );
    ...
    <Modal title={`Draft PO — ${row.name}`} onClose={onClose}>
      ...
      >
        {[
          ...
          [
            "Days of Cover",
            row.daysOfCover != null ? `${fmtN(row.daysOfCover)} days` : "—",
          ],
          ...
          [
            "Paper Type",
            row.paperType
              ? `${row.paperType} ${row.gsm ? row.gsm + " GSM" : ""}`
              : "—",
          ],
        ].map(([label, val]) => (
          <div key={label}>
...
export default function RMForecast({
  rawStock = [],
  jobOrders = [],
  vendorMaster = [],
  toast,
  ...
  const rows = useMemo(() => {
    return (rawStock || [])
      ...
      .map((item) => {
        ...
        else if (daysOfCover != null && daysOfCover < threshold)
          status = "critical";
        else if (daysOfCover != null && daysOfCover < threshold * 2)
          status = "warning";
        else if (v30 > 0 || currentKg > 0) status = "ok";
        else status = "nodata";

        const suggestedKg =
          v30 > 0 ? Math.max(0, (threshold + 7) * v30 - currentKg) : 0;
      ...
      .filter((r) => {
        ...
        if (search) {
          ...
          if (
            ...
            !(r.paperType || "").toLowerCase().includes(q)
          )
            return false;
        }
        return true;
  ...
  return (
    <div className="fade">
        ...
        >
          ...
          >
            ...
            <input
              ...
              value={bufferDays}
              onChange={(e) =>
                setBufferDays(Math.max(0, Number(e.target.value)))
              }
              style={{
                width: 52,
                padding: "5px 8px",
        ...
        {filtered.length === 0 ? (
          ...
          >
            ...
            <span style={{ fontSize: 11, display: "block", marginTop: 6 }}>
              Stock items need a <b>Paper Type</b> and <b>GSM</b> that match
              your Job Orders for velocity to be computed.
            </span>
          </div>
        ) : (
          <div style={{
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr>
                  <th
                    style={{ ...thStyle("name"), textAlign: "left" }}
                  ...
                  <th
                    style={{ ...thStyle("paperType"), textAlign: "left" }}
                  ...
                  <th
                    style={thStyle("currentKg")}
                    onClick={() => handleSort("currentKg")}
                  >
                    STOCK (kg)
                    {sortKey === "currentKg"
                      ? sortDir === "asc"
                  ...
                  <th style={thStyle("v30")} onClick={() => handleSort("v30")}>
                    30d vel (kg/d)
                    {sortKey === "v30" ? (sortDir === "asc" ? " ↑" : " ↓") : ""}
                  </th>
                  <th style={thStyle("v60")} onClick={() => handleSort("v60")}>
                    60d vel (kg/d)
                    {sortKey === "v60" ? (sortDir === "asc" ? " ↑" : " ↓") : ""}
                  </th>
                  <th style={thStyle("v90")} onClick={() => handleSort("v90")}>
                    90d vel (kg/d)
                    {sortKey === "v90" ? (sortDir === "asc" ? " ↑" : " ↓") : ""}
                  </th>
                  <th
                    style={thStyle("daysOfCover")}
                  ...
                  <th
                    style={{ ...thStyle("status"), textAlign: "center" }}
                    onClick={() => handleSort("status")}
                  >
                    STATUS
                  </th>
                  <th
                    style={thStyle("suggestedKg")}
                    onClick={() => handleSort("suggestedKg")}
                  >
                    SUGGESTED ORDER
                    {sortKey === "suggestedKg"
                      ? sortDir === "asc"
                  ...
                  <th style={{ ...thStyle("action"), cursor: "default" }}>
                    ACTION
                  </th>
                </tr>
              </thead>
                    ...
                    >
                      ...
                      <td
                        style={{
                          ...tdR,
                          fontWeight: 500,
                          color: r.currentKg > 0 ? "#fff" : C.muted,
                        }}
                      >
                        {r.currentKg > 0
                      ...
                      {[r.v30, r.v60, r.v90].map((v, vi) => (
                        <td
                          key={vi}
                          style={{ ...tdR, color: v > 0 ? C.text : C.muted }}
                        >
                          {v > 0 ? fmtN(v) : "—"}
                        </td>
                      ))}
                      ...
                      {}
                      <td
                        style={{ ...tdR, fontWeight: 500, color: coverColor }}
                      >
                        {r.daysOfCover != null ? (
                          <>
                            {fmtN(r.daysOfCover)}{" "}
                      ...
                      <td
                        style={{
                          ...tdR,
                          color: r.suggestedKg > 0 ? "#f59e0b" : C.muted,
                          fontWeight: r.suggestedKg > 0 ? 700 : 400,
                        }}
                      >
                        {r.suggestedKg > 0
                          ? `${fmtN(r.suggestedKg, 0)} kg`
                          : "—"}
                      </td>
      ...
      {}
      {summary.dead > 0 &&
        (statusFilter === "all" || statusFilter === "dead") && (
          <div
            style={{
              marginTop: 16,
              padding: "14px 18px",
          ...
          >
            ...
            >
              ⚫ Dead Stock Alert — {summary.dead} item
              {summary.dead > 1 ? "s" : ""}
            </div>
            <div style={{ fontSize: 12, color: C.muted, lineHeight: 1.6 }}>
              These raw materials have{" "}
              <b>no consumption recorded in the last 90 days</b> via job orders.
              They are tying up working capital. Consider:
              <br />
              • Verifying if paperType + GSM in Job Orders matches these stock
              items
              <br />
              • Writing off / returning to vendor if genuinely unused
              <br />• Repurposing for alternate products
            </div>
          </div>
        )}
      ...
      >
        <b style={{ color: "#555" }}>How velocity is computed:</b> Matches RM
        stock to Job Orders by <b>Paper Type + GSM</b>. Weight per job ={" "}
        <code>reelWeightKg</code> (if set) or{" "}
        <code>noOfSheets × gsm × area(m²) / 1000</code>. Includes Scheduled, In
        Progress, and Completed jobs. Days-of-cover ={" "}
        <code>current stock ÷ 30-day velocity</code>. Critical threshold = lead
        time ({leadDays}d) + buffer ({bufferDays}d) ={" "}
        <b>{leadDays + bufferDays} days</b>. Suggested order covers lead time +
        buffer + 7-day safety stock.
      </div>
