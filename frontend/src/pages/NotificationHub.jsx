...
const loadSettings = () => {
  try {
    return JSON.parse(localStorage.getItem(LS_WA) || "{}");
  } catch {
    return {};
  }
};
...
function EmailTab({
  ...
  const digest = useMemo(() => {
    ...
    const totalYestProduction = yesterdayEntries.reduce(
      (s, e) => s + (e.qtyCompleted || e.good || 0),
      0,
    );
    ...
    const dispatchQty = todayDispatches.reduce(
      (s, d) => s + (d.qty || d.quantity || 0),
      0,
    );
    ...
    const overdue = (salesOrders || []).filter((so) => {
      ...
      return (
        dd &&
        dd < todayStr &&
        !["Completed", "Cancelled", "Dispatched"].includes(so.status)
      );
    });
    ...
    const lowStock = (consumableStock || []).filter(
      (c) => Number(c.qty || 0) <= Number(c.reorderLevel || 0),
    );

    const totalMachines = (activeMachines || []).length;
    const runningToday = new Set(
      (allEntries || [])
        .filter((e) => e.date?.slice(0, 10) === yest)
        .map((e) => e.machineId),
    ).size;

    const pendingPOs = (purchaseOrders || []).filter(
      (p) => !["Received", "Closed"].includes(p.status || ""),
    ).length;
...
${digest.todayDispatches
  .slice(0, 5)
  .map(
    (d) =>
      `  → ${d.dispatchNo || ""} · ${d.clientName || d.client || "Client"} · ${fmtN(d.qty || d.quantity || 0)} units`,
  )
  .join("\n")}
...
${digest.dueThisWeek
  ...
  .map(
    (so) =>
      `  ⏰ ${so.soNo} — ${so.companyName} — Due ${fmtDate(so.deliveryDate)}`,
  )
  .join("\n")}
...
${
  digest.overdue.length > 0
    ? `• OVERDUE ORDERS: ${digest.overdue.length} SO${digest.overdue.length !== 1 ? "s" : ""} past delivery date\n${digest.overdue
        ...
        .map(
          (so) =>
            `  ❌ ${so.soNo} — ${so.companyName} — Was due ${fmtDate(so.deliveryDate)}`,
        )
        .join("\n")}`
    : "• No overdue orders ✅"
}
${
  digest.lowStock.length > 0
    ? `• LOW STOCK: ${digest.lowStock.length} consumable${digest.lowStock.length !== 1 ? "s" : ""} at or below reorder level\n${digest.lowStock
        ...
        .map(
          (c) =>
            `  ⚠️  ${c.name || c.itemName} — ${c.qty} ${c.unit || "units"} remaining`,
        )
        .join("\n")}`
  ...
  return (
    <div>
      <div style={{ fontSize: 17, fontWeight: 800, marginBottom: 4 }}>
        Executive Morning Email
      </div>
      <div style={{ fontSize: 12, color: "#888", marginBottom: 20 }}>
        Daily factory digest — production, dispatch, alerts. Set up your email
        client to send at 8am.
      </div>
      ...
      >
        ...
        {!recipientEmail && (
          <div style={{ fontSize: 11, color: "#555", marginTop: 8 }}>
            Enter recipient email to enable "Open in Mail App". Actual scheduled
            sending requires backend email service (e.g. SendGrid, Resend, or
            SMTP).
          </div>
        )}
      </div>
...
function WhatsAppTab({ salesOrders, dispatches, purchaseOrders, toast }) {
  ...
  const openPOs = useMemo(
    () =>
      (purchaseOrders || [])
        .filter((p) => !["Received", "Closed"].includes(p.status || ""))
        .slice()
        .reverse()
        .slice(0, 20),
    [purchaseOrders],
  ...
  const generateDispatchMsg = () => {
    const d = dispatches.find(
      (x) => x.dispatchNo === selectedDispatch || x._id === selectedDispatch,
    );
    if (!d) {
      toast?.("Select a dispatch record", "error");
      return;
    }
    ...
    const date = new Date(
      d.dispatchDate || d.date || Date.now(),
    ).toLocaleDateString("en-GB");
    const items =
      (d.items || [])
        .map((i) => `  • ${i.itemName || i.name}: ${i.qty || i.quantity} pcs`)
        .join("\n") || `  • ${qty} units`;
...
${settings.companyName || "Your Packaging Partner"}`;
  ...
  const generatePOAckMsg = () => {
    const po = purchaseOrders.find(
      (x) => x.poNo === selectedPO || x._id === selectedPO,
    );
    if (!po) {
    ...
    const vendor =
      typeof po.vendor === "object"
        ? po.vendor.name
        : po.vendor || po.vendorName || "Vendor";
    ...
    const items =
      (po.items || [])
        .map(
          (i) =>
            `  • ${i.name || i.itemName}: ${(i.qty || i.quantity || 0).toLocaleString("en-IN")} ${i.unit || "units"} @ ₹${i.unitPrice || i.rate || 0}/unit`,
        )
        .join("\n") || "  • As per PO";
...
${settings.companyName || "Purchase Team"}`;
  ...
  const generateDailySummary = () => {
    ...
    const todayDisp = (dispatches || []).filter(
      (d) => (d.dispatchDate || d.date || "").slice(0, 10) === todayStr,
    );
    const dueThisWeek = (salesOrders || []).filter((so) => {
      const dd = so.deliveryDate?.slice(0, 10);
      const inWeek =
        dd &&
        dd >= todayStr &&
        dd <= new Date(Date.now() + 7 * 86400000).toISOString().slice(0, 10);
...
${dueThisWeek
  ...
  .map(
    (so) =>
      `• ${so.soNo} — ${so.companyName} — ${new Date(so.deliveryDate).toLocaleDateString("en-GB")}`,
  )
  .join("\n")}
...
— ManufactureIQ`;
  ...
  const TEMPLATES = [
    {
      ...
      desc: "Notify client when goods are dispatched",
    },
    {
      id: "po_ack",
      label: "PO Acknowledgment",
      icon: "fa-solid fa-clipboard-list",
      desc: "Confirm PO to vendor with delivery expectations",
    },
    {
      id: "daily",
      label: "Daily Owner Summary",
      icon: "fa-solid fa-chart-bar",
      desc: "Morning briefing to owner/manager",
    },
  ];
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 12, color: "#888", marginBottom: 20 }}>
        Generate ready-to-send WhatsApp messages. Opens WhatsApp Web with
        pre-filled text — no API required.
      </div>
        ...
        >
          <div>
            ...
            <input
              ...
              onChange={(e) => saveWASettings({ companyName: e.target.value })}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => saveWASettings({ ownerPhone: e.target.value })}
        ...
        {TEMPLATES.map((t) => (
          <button
            ...
            style={{
              ...
              border: `1px solid ${activeTemplate === t.id ? "rgba(255,255,255,0.25)" : "rgba(255,255,255,0.1)"}`,
              borderRadius: 10,
              cursor: "pointer",
              textAlign: "left",
          ...
          >
            ...
            <div
              style={{
                fontWeight: 500,
                fontSize: 13,
                color: activeTemplate === t.id ? "#fff" : "#e0e0e0",
                marginBottom: 4,
              }}
            >
              {t.label}
                ...
                >
                  ...
                  {recentDispatches.map((d) => (
                    <option key={d._id} value={d.dispatchNo || d._id}>
                      {d.dispatchNo} — {d.clientName || d.client} —{" "}
                      {(d.dispatchDate || d.date || "").slice(0, 10)}
                    </option>
                  ))}
                </select>
              </div>
                  ...
                  {openPOs.map((p) => {
                    const vName =
                      typeof p.vendor === "object"
                        ? p.vendor.name
                        : p.vendor || p.vendorName;
                    return (
                      <option key={p._id} value={p.poNo || p._id}>
                        {p.poNo} — {vName}
                      </option>
                    );
                  })}
          ...
          <div>
            ...
            >
              <div style={{ flex: 1, minWidth: 180 }}>
                ...
                <input
                  value={customPhone || settings.ownerPhone || ""}
                  onChange={(e) => setCustomPhone(e.target.value)}
                  placeholder="10-digit"
                  style={inp}
                />
      ...
      <div style={{ fontSize: 11, color: "#555", padding: "10px 0" }}>
        WhatsApp Web links open wa.me on your browser — no API key required. For
        automated/scheduled sending, integrate WhatsApp Business API (Meta) with
        your backend.
      </div>
    </div>
  );
