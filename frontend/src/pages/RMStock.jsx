      ...
      try {
        ...
        const imported = rows.map((row) => {
          ...
          return {
            ...
            reorderLevel: parseFloat(obj["reorder (kg)"] || obj["reorderlevel"] || 0),
    ...
    <div className="fade">
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginBottom: 12 }}>
        ...
        <ActionBtn label="↓ Template" color="#6366f1" onClick={() => {
          const csv = "Code,Material Name,Category,Qty (Sheets),Qty (KG),Reorder (KG),Rate (₹/KG)\n,,,,,,";
          const blob = new Blob([csv], { type: "text/csv" });
          const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "rm_stock_template.csv"; a.click();
        }} />
        <ActionBtn label="↑ Import Excel" color={C.blue || "#3b82f6"} onClick={() => fileInputRef.current?.click()} />
        ...
        {categories.map((cat) => (
          <button
            ...
            style={{
              ...
              background: activeFilter === cat ? C.blue || "#3b82f6" : "transparent",
      ...
      >
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${C.border}` }}>
                {["CODE", "MATERIAL NAME", "CATEGORY", "QTY (SHEETS)", "QTY (KG)", "REORDER\n(KG)", "RATE (₹/KG)", "VALUE (₹)", "ACTION"].map((h) => (
                  <th
                    key={h}
                    style={{
                      padding: "11px 14px",
                      textAlign: h.includes("QTY") || h.includes("RATE") || h.includes("VALUE") ? "right" : "left",
                      fontSize: 11,
                      fontWeight: 700,
                ...
                filtered.map((s, i) => {
                  ...
                  return (
                    <tr key={s.id || i} style={{ borderBottom: `1px solid ${C.border}22`, background: i % 2 === 1 ? (C.inputBg || "#ffffff08") : "transparent" }}>
                      <td style={{ padding: "10px 14px", color: C.muted, fontSize: 12, fontFamily: "'JetBrains Mono',monospace" }}>{s.code || "—"}</td>
                      ...
                      <td style={{ padding: "10px 14px", color: C.muted, fontSize: 12 }}>{s.category || s.paperCategory || "—"}</td>
                      <td style={{ padding: "10px 14px", textAlign: "right", fontFamily: "'JetBrains Mono',monospace", fontWeight: 600 }}>{fmt(qty) || "—"}</td>
                      <td style={{ padding: "10px 14px", textAlign: "right", fontFamily: "'JetBrains Mono',monospace" }}>{fmt(Math.round(weightKg)) || "—"}</td>
                      <td style={{ padding: "10px 14px", textAlign: "right", color: C.muted }}>{reorder ? fmt(reorder) : "—"}</td>
                      <td style={{ padding: "10px 14px", textAlign: "right", color: C.muted }}>{rate ? `₹${fmt(rate)}` : "—"}</td>
                      <td style={{ padding: "10px 14px", textAlign: "right", fontWeight: 600, color: C.green || "#22c55e" }}>{value ? `₹${fmt(Math.round(value))}` : "—"}</td>
                      <td style={{ padding: "10px 14px" }}>
                        <button
                          onClick={() => {
                            setRawStock((prev) => prev.filter((_, idx) => idx !== i));
        ...
        >
          ...
          <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
            {[["#ef4444", "Low"], ["#f97316", "Moderate"], ["#22c55e", "Adequate"]].map(([color, label]) => (
