...
const card = (col) => ({
  background: "#111",
  border: `1px solid ${col || "#2a2a2a"}22`,
  borderRadius: 10,
  padding: 20,
});
...
function FAITab({ jobOrders, toast }) {
  ...
  const handleSubmit = () => {
    if (!form.joRef) {
      toast?.("Select a job order", "error");
      return;
    }
  ...
  return (
    <div>
      ...
      {showModal && (
        <Modal
          title={editId ? "Edit FAI Record" : "New First Article Inspection"}
          onClose={() => {
            setForm(blankForm);
            setEditId(null);
            setShowModal(false);
          ...
          <div>
            ...
            >
              <div>
                ...
                >
                  ...
                  {joOptions.map((jo) => (
                    <option key={jo._id} value={jo.joNo}>
                      {jo.joNo} — {jo.companyName} · {jo.itemName}
                    </option>
                  ))}
                </select>
              </div>
              <div>
                ...
                <input
                  value={form.sampleNo}
                  onChange={(e) => setF("sampleNo", e.target.value)}
                  style={inp}
                  placeholder="e.g. 1"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setF("inspectorName", e.target.value)}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setF("approverName", e.target.value)}
                  placeholder="Name of approver / sign-off"
                  style={inp}
                />
              </div>
              <div>
                ...
                <select
                  ...
                  style={{
                    ...inp,
                    color: FAI_STATUS[form.status]?.color || "#e0e0e0",
                    fontWeight: 700,
                  }}
                >
                  ...
                  >
                    ...
                    <button
                      onClick={() =>
                        setF(
                          "measurements",
                          form.measurements.filter((_, j) => j !== i),
                        )
                      }
                      style={{
                        background: "transparent",
            ...
            >
              <div>
                ...
                <textarea
                  value={form.defects}
                  onChange={(e) => setF("defects", e.target.value)}
                  placeholder="List any defects or non-conformances..."
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Additional notes, conditional approvals..."
                  style={{ ...inp, height: 80, resize: "vertical" }}
                />
              </div>
            </div>
              ...
              >
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  ...
                  <button
                    onClick={() => {
                      if (confirm("Delete?")) {
                        const recs = records.filter((x) => x.id !== r.id);
                        persist(recs);
                      }
                    }}
                    style={{
      ...
      {}
      <div
        style={{ marginTop: 16, ...card("#f59e0b"), borderColor: "#f59e0b33" }}
      >
        <div
          style={{
            fontWeight: 500,
...
function SupplierTab({ purchaseOrders, inward, vendorMaster, toast }) {
  ...
  const scores = useMemo(() => {
    ...
    (inward || []).forEach((g) => {
      ...
      if (po) {
        if (!inwardByPO[po]) inwardByPO[po] = [];
        inwardByPO[po].push(g);
      }
    });

    return Object.entries(vendorPOs)
      .map(([vendor, pos]) => {
        let onTimeCount = 0,
          lateCount = 0,
          totalPOs = pos.length;
        ...
        pos.forEach((po) => {
          ...
          const grnDate = grns.length
            ? new Date(
                Math.min(
                  ...grns.map((g) => +new Date(g.inwardDate || g.createdAt)),
                ),
              )
            : null;
          if (grnDate) {
            if (grnDate <= dueDate) onTimeCount++;
            else lateCount++;
          }
        });
        ...
        const onTimePct =
          deliveredPOs > 0
            ? Math.round((onTimeCount / deliveredPOs) * 100)
            : null;
        ...
        const responsiveness =
          manual?.responsiveness != null ? Number(manual.responsiveness) : null;
        ...
        const composite =
          rawScore != null ? Math.min(100, Math.round(rawScore)) : null;
        ...
        const action =
          grade === "A"
            ? "Concentrate business — preferred vendor"
            : grade === "B"
              ? "Monitor — improvement plan in place"
              : grade === "C"
                ? "Review — shortlist replacements"
  ...
  const saveResponsiveness = () => {
    ...
    updated.push({
      vendor: editVendor,
      responsiveness: Number(rForm.responsiveness),
      notes: rForm.notes,
    });
    save(LS_VQ, updated);
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 12, color: "#888", marginBottom: 20 }}>
        Computed from {purchaseOrders?.length || 0} POs · {inward?.length || 0}{" "}
        GRNs · Click a vendor to enter responsiveness score
      </div>
      ...
      >
        {["A", "B", "C"].map((g) => {
          ...
          return (
            <div
              key={g}
              style={{ ...card(GRADE_COLORS[g]), textAlign: "center" }}
            >
              <div
                style={{
                  fontSize: 36,
          ...
          >
            <div style={{ flex: 1, minWidth: 180 }}>
              ...
              <input
                ...
                min={0}
                max={100}
                value={rForm.responsiveness}
                onChange={(e) =>
                  setRForm((f) => ({ ...f, responsiveness: e.target.value }))
                }
                style={inp}
                placeholder="e.g. 85"
              />
            ...
            <div style={{ flex: 2, minWidth: 200 }}>
              ...
              <input
                value={rForm.notes}
                onChange={(e) =>
                  setRForm((f) => ({ ...f, notes: e.target.value }))
                }
                style={inp}
                placeholder="e.g. replies same day, tracks PO proactively"
              />
            </div>
            <button
              onClick={saveResponsiveness}
              style={{
                background: "transparent",
                color: "#ffffff",
                border: "1px solid rgba(255,255,255,0.2)",
          ...
          <tbody>
            {scores.length === 0 && (
              <tr>
                ...
                >
                  No purchase orders found. Supplier scores are computed
                  automatically from PO + GRN data.
                </td>
              </tr>
            )}
            {scores.map((s, i) => (
              <tr
                ...
                onClick={() => {
                  ...
                  setRForm({
                    ...
                    notes:
                      manualScores.find((m) => m.vendor === s.vendor)?.notes ||
                      "",
                  });
                }}
              >
                ...
                <td
                  style={{ padding: "12px 12px", fontSize: 12, color: "#888" }}
                >
                  {s.totalPOs}
                </td>
                <td style={{ padding: "12px 12px" }}>
                ...
                <td
                  style={{ padding: "12px 12px", fontSize: 11, color: "#aaa" }}
                >
                  {s.action}
                </td>
              </tr>
      ...
      <div style={{ fontSize: 11, color: "#555", marginTop: 12 }}>
        Score = On-Time Delivery (40%) + Responsiveness (30%) + remaining
        weighted by data available. A ≥ 70 · B 40–69 · C &lt; 40. Click any row
        to enter responsiveness score manually.
      </div>
    </div>
  );
}
