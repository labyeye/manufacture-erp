...
export const setUsers = (v) =>
  localStorage.setItem("erp_users", JSON.stringify(v));
...
export const setRoles = (v) =>
  localStorage.setItem("erp_roles", JSON.stringify(v));
...
export const computeConsumableItemName = (it) => {
  ...
  return (
    (it.category && it.category.match(/Box|Bag|Polybag/i)
      ? it.category + dimStr
      : it.category) + (it.uom ? " (" + it.uom + ")" : "")
  );
};
...
export function buildSchedule(form, machineMaster, existingJobOrders = []) {
  ...
  procs.forEach((proc, idx) => {
    const allocMachines =
      proc.machineIds && proc.machineIds.length > 0
        ? (machineMaster || []).filter(
            (m) => proc.machineIds.includes(m.id) && m.status === "Active",
          )
        : ["Formation", "Manual Formation"].includes(proc) &&
            proc !== "Formation"
          ? (machineMaster || []).filter(
              (m) =>
                MANUAL_FORMATION_MACHINES.includes(m.type) &&
                m.status === "Active",
            )
          : ["Formation", "Manual Formation"].includes(proc)
            ? (machineMaster || []).filter(
                (m) =>
                  FORMATION_MACHINES.includes(m.type) && m.status === "Active",
              )
            : (machineMaster || []).filter(
                (m) => m.type === proc && m.status === "Active",
              );
    ...
    let duration =
      form.durByProcess && form.durByProcess[proc]
        ? +form.durByProcess[proc]
        : 1;
